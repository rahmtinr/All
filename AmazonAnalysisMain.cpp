...
int main(int argc, char *argv[]) {
	...
	string filename = Amazon::Global::output_directory + "current_relative_year_usage_bucketed_" + SimpleIntToString(each_bucket)  +"_top" + SimpleIntToString(SIZE_OF_TOP_INNOVATIONS) + "_innovations_coeff" + SimpleDoubleToString(Amazon::Global::state_coeffecient) + ".txt";
	...
	for(int i = 0; i < 2 * shifter; i++) {
		...
		for(int j = 0; j < relative_alphas[i].size(); j++){
