...
xp_distribution$CurrentBucket = floor(xp_distribution$CurrentExp / 10)
xp_distribution$FinishingBucket = floor(xp_distribution$FinishingExp / 10)
...
my_summary 
pd = position_dodge(.1)
jpeg(paste(directory,"AggregationPlots/", dataset, "_mean_appereance_of_each_xp.jpg",sep=""), height=800, width=1300)
ggplot(data = my_summary, aes(x = CurrentBucket, y=mean, colour = "blue")) + 
    geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1, position=pd) +
    geom_line(position=pd) +  ggtitle("Mean time vs Experience bucket of size 10") +
